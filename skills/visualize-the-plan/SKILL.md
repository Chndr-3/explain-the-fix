...
2. **Classify the shape** — see `references/classification.md` for the heuristic (step count, whether steps branch on a decision, whether the plan is really about which files/modules will change, whether phases group naturally). Default to the smaller shape when ambiguous.
3. **Build the node list** — max 8 nodes total. If the plan has more steps than that, group a cluster of related steps into one node ("wire up the three API routes") rather than enumerating every one.
