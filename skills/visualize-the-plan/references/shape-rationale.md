...
3 nodes, vertical: Goal → Approach → Why this way. Use when the user asks *why* you're planning to do it this way, not just what the steps are — the plan equivalent of `visualize-the-fix`'s deep dive.
...
- Same vertical `.spine` connector as `visualize-the-fix`'s deep dive (short dashed line, not an arrow-flow — this tier is a drill-down, not a sequence).
