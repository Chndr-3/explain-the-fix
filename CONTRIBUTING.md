...
Thanks for considering a contribution. This repo hosts three small skills that share one house style — most changes are edits to markdown or a reference HTML file, no build step involved.

## Project layout

```
skills/
  visualize-the-problem/  — diagrams a confirmed bug/incident root cause
    SKILL.md
    references/
  visualize-the-plan/     — diagrams a proposed plan before it's built
    SKILL.md
    references/
  visualize-the-fix/      — diagrams a just-finished coding task
    SKILL.md
    references/
docs/                     — example SVGs used in README.md
```

Each skill is self-contained: its own `SKILL.md`, its own `references/renderer.md` (rendering spec), `references/style-reference.html` (canonical working example), `references/glossary.md` (jargon dictionary), and its own shape-classification/shape-detail docs. None of the three depend on each other at runtime — the shared house style is duplicated on purpose so any one can be installed alone.
...
1. **Style/layout changes** — edit the skill's `references/style-reference.html` directly, open it in a browser to confirm it still looks and behaves correctly (theme toggle, glossary modal + drawer, all shapes), then update `renderer.md`'s prose to match what you changed. If the change should apply to all three skills (e.g. a token color fix), make the same edit in `skills/visualize-the-problem/references/style-reference.html`, `skills/visualize-the-plan/references/style-reference.html`, and `skills/visualize-the-fix/references/style-reference.html` — they're intentionally independent copies, not a shared include.
...
3. **New glossary terms** — add to the skill's `references/glossary.md` rather than only defining a term inline in one diagram; that way it's reused next time. The three skills keep separate glossaries since their jargon skews differently (debugging vocabulary vs. planning/rollout vocabulary vs. implementation detail) — add a term to more than one only if it's genuinely common across them.
...
- Keep diagrams to the existing budget: 8 nodes max, one diagram per explanation/plan/diagnosis. Don't add a new shape without updating both that skill's `SKILL.md` table and its `renderer.md`.
- Each skill's reserved-accent node treatment means something different — don't mix them up: `visualize-the-fix`'s `.box-done`/check-tick means "already resolved," `visualize-the-plan`'s `.box-goal`/target-mark means "proposed, not built," and `visualize-the-problem`'s `.box-problem`/warning-mark means "confirmed, still unresolved." Never use one skill's treatment in another's diagrams.

## Reporting bugs / requesting shapes

Open a [GitHub issue](https://github.com/Chndr-3/visualize-skills/issues). For a bug, include the diff/plan/diagnosis and which skill produced the bad diagram if you can — most rendering bugs are about a specific label/shape combination, not the renderer in general.
